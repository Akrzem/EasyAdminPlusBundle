...
namespace Akrzem\EasyAdminPlusBundle\Acl\Security;
...
class AdminAuthorizationChecker
{
    ...
    public function checksUserAccess(array $entity, string $actionName)
    {
        ...
        if (is_array($requiredRole)){
            $grant = false;
            foreach($requiredRole as $foo){
                if ($foo && $this->authorizationChecker->isGranted($foo)) {
                    $grant = true;
                }
            }
            if (!$grant){
                throw new AccessDeniedException(
                    sprintf('You must be granted %s role to perform this entity action !', 'rola')
                );
            }
